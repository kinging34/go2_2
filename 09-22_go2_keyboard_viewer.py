...
PARAMS_DIR = "params_09-23_lider_"
...
START_TERRAIN = "hard"                       # flat / easy / medium / hard
...
residual_path = Path(RESIDUAL_CHECKPOINT) if RESIDUAL_CHECKPOINT else newest(
    f"{RESIDUAL_MODEL_NAME}_*_*it.pt"
)
...
print(f"obs={base_actor.obs_dim}, action={base_actor.act_dim}, residual_scale={residual_scale}")
...
class KeyboardPolicy:
    def __init__(self, prior, mean, variance, residual=None, scale=0.25, stateful=True):
        self.prior = prior
