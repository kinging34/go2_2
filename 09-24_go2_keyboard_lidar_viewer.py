#!/usr/bin/env python3
"""WSL/WSLgでGo2の学習済みSNNをキーボード操作するレイLiDAR対応ビューア。
...
RESIDUAL_CHECKPOINT = None                    # 例: WORK_DIR / "params_09-24/go2_residual_ray_lidar_stateful_10000it.pt"

BASE_MODEL_NAME = "go2_imitation"
BASE_NORM_NAME = "go2_imitation_norm"
RESIDUAL_MODEL_NAME = "go2_residual_ray_lidar"
...
LIDAR_SENSOR_PREFIX = "research_lidar_ray_"


def lidar_ray_layout(spec):
    mount = np.asarray(spec["mount"], dtype=np.float64)
    targets = np.array([(x, y, -spec["nominal_body_height"])
                        for x in spec["x"] for y in spec["y"]], dtype=np.float64)
    vectors = targets - mount
    nominal = np.linalg.norm(vectors, axis=1)
    if np.any(nominal <= 0) or np.any(nominal >= spec["max_range"]):
        raise ValueError("LiDAR の走査範囲が max_range を超えています")
    return vectors / nominal[:, None], nominal


def create_lidar_scene(scene_xml, output_xml, spec):
    scene_path = Path(scene_xml).resolve()
    output_path = Path(output_xml).resolve()
    if output_path.parent != scene_path.parent:
        raise ValueError("LiDAR 用XMLは元の scene と同じフォルダへ保存してください")
    scene_tree = ET.parse(scene_path)
    scene_root = scene_tree.getroot()
    includes = [item for item in scene_root.findall("include")
                if Path(item.get("file", "")).name == "go2.xml"]
    if len(includes) != 1:
        raise ValueError("scene に go2.xml の include が1個必要です")
    robot_path = scene_path.parent / includes[0].get("file")
    robot_tree = ET.parse(robot_path)
    robot_root = robot_tree.getroot()
    base = robot_root.find("./worldbody/body[@name='base_link']")
    if base is None:
        raise ValueError("go2.xml に base_link がありません")
    sensors = robot_root.find("sensor")
    if sensors is None:
        sensors = ET.SubElement(robot_root, "sensor")
    directions, _ = lidar_ray_layout(spec)
    mount = " ".join(f"{v:.12g}" for v in spec["mount"])
    for index, direction in enumerate(directions):
        quat = np.array([1.0 + direction[2], -direction[1], direction[0], 0.0])
        if np.linalg.norm(quat) < 1e-8:
            quat = np.array([0.0, 1.0, 0.0, 0.0])
        quat /= np.linalg.norm(quat)
        name = f"{LIDAR_SENSOR_PREFIX}{index:02d}"
        ET.SubElement(base, "site", name=name, pos=mount,
                      quat=" ".join(f"{v:.12g}" for v in quat), size="0.003")
        ET.SubElement(sensors, "rangefinder", name=name, site=name)
    robot_output = scene_path.parent / "go2_research_lidar.xml"
    ET.indent(robot_tree, space="  ")
    robot_tree.write(robot_output, encoding="utf-8", xml_declaration=True)
    includes[0].set("file", robot_output.name)
    ET.indent(scene_tree, space="  ")
    scene_tree.write(output_path, encoding="utf-8", xml_declaration=True)
    return output_path


def lidar_features(distances, directions, nominal, spec):
    valid = np.isfinite(distances) & (distances > 0) & (distances <= spec["max_range"])
    measured = np.where(valid, distances, nominal)
    height = np.clip((nominal - measured) * (-directions[:, 2]) /
                     spec["height_scale"], -1.0, 1.0)
    height = np.where(valid, height, 0.0)
    return np.concatenate([height, valid.astype(np.float64)]).astype(np.float32)
...
if residual_path is not None and residual_path.is_file():
    ...
    lidar_spec = residual_checkpoint.get("lidar_spec")
    if lidar_spec is None:
        raise ValueError("旧形式の残差checkpointです。レイLiDAR入力で再学習してください")
    residual_actor = actor_from_state(residual_checkpoint["residual_actor"])
    ray_count = len(lidar_spec["x"]) * len(lidar_spec["y"])
    if residual_actor.obs_dim != base_actor.obs_dim + base_actor.act_dim + 2 * ray_count:
        raise ValueError(
            f"残差actorの入力次元が不一致です: {residual_actor.obs_dim} != "
            f"{base_actor.obs_dim} + {base_actor.act_dim} + {2 * ray_count}"
        )
...
if lidar_spec is not None:
    SCENES = {
        name: create_lidar_scene(path, path.with_name(path.stem + "_lidar.xml"), lidar_spec)
...
class KeyboardPolicy:
    def __init__(
        ...
        residual_limit=0.50, stateful=True, lidar_spec=None
    ):
        self.prior = prior
    ...
    def act(self, observation, lidar=None):
        ...
        if self.use_residual:
            if lidar is None:
                raise ValueError("残差ActorにはLiDAR計測値が必要です")
            lidar_tensor = torch.as_tensor(lidar, dtype=torch.float32,
                                           device=TORCH_DEVICE).unsqueeze(0)
            residual_input = torch.cat([norm, prior_action, lidar_tensor], dim=-1)
...
""")
    ...
    def run_scene(self):
        ...
        lidar_addresses = None
        lidar_directions = lidar_nominal = None
        if self.policy.lidar_spec is not None:
            lidar_directions, lidar_nominal = lidar_ray_layout(self.policy.lidar_spec)
            lidar_addresses = np.array([
                self.sensor_address(model, f"{LIDAR_SENSOR_PREFIX}{index:02d}")
                for index in range(len(lidar_nominal))
            ], dtype=np.int64)
        ...
        try:
            while (
                ...
                if not self.paused and now >= next_control:
                    ...
                    lidar = None
                    if lidar_addresses is not None:
                        distances = data.sensordata[lidar_addresses]
                        lidar = lidar_features(distances, lidar_directions,
                                               lidar_nominal, self.policy.lidar_spec)
                    action, self.last_residual_rms = self.policy.act(obs, lidar)
